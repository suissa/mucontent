...
function auth_yet(req, res, next) {
	if (req.session.info) {
		...
		rendering(req.headers.host, '404', data, req.session.connected, function callback(layout) {
			res.end(layout);
		});
	} else {
		next();
...
// check if the role is enabled to see the module
			acl = acl.split(',');
			if (req.session.info) {
				var role = req.session.info[0].role, find = false;
