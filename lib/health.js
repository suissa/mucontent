...
// remove from database
	...
	if (configuration.Params.network_configuration_command && (appserver_list[0] == cache.get('client_ip'))) {
		var options = {
		...
		var request = http.request(options, function (res) {
			utils.quicklog("Checl sull'ip del proxy " + options.host + " status: " + res.statusCode);
		});
		request.on('error', function(error) {
			...
			for (var i = 0; i < numCPUs; i++) {
				...
				// refork the process if one exit
				cluster.fork();
			});
		...
		var alias=0;
 		ifaces[dev].forEach(function(details){
				...
				if (dev === configuration.Params.listen_client_interface) {
					...
					// put in cache
					cache.put('client_ip', details.address);
